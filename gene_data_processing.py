...
def reduce_genome(
    ...
    if reduction_method == 'nn':
        ...
        reduced_genes_mat = get_neural_net_positions(
            genome_data=genes_matrix, model_type='simple')
    ...
    if reduced_genes_mat is not None:
        ...
        if reduction_type == 'color':
            ...
            reduced_genes_mat -= reduced_genes_mat[genome_id_to_index[1]]
...
def apply_rotation(positions_matrix: np.ndarray, genome_id_to_index: dict, best_genome_id):
    ...
    best_genome_magnitude = np.linalg.norm(best_genome)

    theta = np.arctan2(float(best_genome[1]), float(best_genome[0]))
...
def get_all_distances(
    ...
    min = int(processing_time / 60)
    sec = int(int(processing_time) % 60)

    print(f"took {min}m, {sec}s")
